...
public class AppRate {
	 ...
	 * 
	 * @param textRes
	 *            text ressource to be displayed
	 * @return the {@link AppRate} instance
	 */
	 ...
	 * 
	 * @param policy
	 *            the {@link RetryPolicy} to be used
	 * @return the {@link AppRate} instance
	 */
	...
	/**
	 * Add a constraint to show the view only if the app is installed for more
	 * than
	 * 
	 * @param installedSince
	 *            the time in seconds
	 * @return the {@link AppRate} instance
	 */
	 ...
	 * 
	 * @param theme
	 *            the {@link fr.nicolaspomepuy.discreetapprate.AppRateTheme} to
	 *            be used
	 * @return the {@link AppRate} instance
	 */
	...
	/**
	 * Pause duration after a crash (in sec.) /!\ Calling
	 * {@link #initExceptionHandler(android.content.Context)} is mandatory to
	 * make it work. You should do it in your {@link android.app.Application}
	 * class
	 * 
	 * @param pauseAfterCrash
	 *            the time to pause
	 * @return the {@link AppRate} instance
	 */
	...
	public void checkAndShow() {
		...
		if (debug)
			LogD("Last crash: "
					+ ((System.currentTimeMillis() - settings.getLong(
							KEY_LAST_CRASH, 0L)) / 1000) + " seconds ago");
		...
		Date installDate = Utils.installTimeFromPackageManager(
				activity.getPackageManager(), activity.getPackageName());
		Date now = new Date();
		if (now.getTime() - installDate.getTime() < installedSince * 1000) {
			if (debug)
				LogD("Date not reached. Time elapsed since installation (in sec.): "
						+ ((now.getTime() - installDate.getTime()) / 1000));
			return;
		}
		...
		if (count == initialLaunchCount) {
			...
			showAppRate();
		} else if (policy == RetryPolicy.INCREMENTAL
				&& count % initialLaunchCount == 0) {
			if (debug)
				LogD("initialLaunchCount incremental reached");
			showAppRate();
		} else if (policy == RetryPolicy.EXPONENTIAL
				&& count % initialLaunchCount == 0
				&& Utils.isPowerOfTwo(count / initialLaunchCount)) {
			if (debug)
				LogD("initialLaunchCount exponential reached");
			showAppRate();
		} else {
			if (debug)
				LogD("Nothing to show. initialLaunchCount: "
						+ initialLaunchCount + " - Current count: " + count);
		}
	}
	...
	public void endMonitoring() {
		...
		editor.putLong(
				KEY_MONITOR_TOTAL,
				settings.getLong(KEY_MONITOR_TOTAL, 0)
	...
	private void showAppRate() {
		final ViewGroup mainView = (ViewGroup) activity.getLayoutInflater()
				.inflate(R.layout.app_rate, null);

		ImageView close = (ImageView) mainView.findViewById(R.id.close);
		TextView textView = (TextView) mainView.findViewById(R.id.text);
		RelativeLayout container = (RelativeLayout) mainView
				.findViewById(R.id.container);
		if (fromTop) {
			FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) container
					.getLayoutParams();
	...
	private void displayViews(ViewGroup mainView) {
		activity.addContentView(mainView, new ViewGroup.LayoutParams(
				ViewGroup.LayoutParams.MATCH_PARENT,
				ViewGroup.LayoutParams.MATCH_PARENT));

		Animation fadeInAnimation;
		if (fromTop) {
			fadeInAnimation = AnimationUtils.loadAnimation(activity,
					R.anim.fade_in_from_top);
		} else {
			fadeInAnimation = AnimationUtils.loadAnimation(activity,
					R.anim.fade_in);
		}
