...
public class GoogleNavigationDrawerAdapter extends BaseAdapter {
	...
	public GoogleNavigationDrawerAdapter(Context context,
			String[] mainSections, String[] secondarySections,
			int[] mainSectionsDrawableIds, int[] secondarySectionsDrawableIds) {
		this(context);
	...
	public GoogleNavigationDrawerAdapter(Context context,
			String[] mainSections, String[] secondarySections,
			int[] mainSectionsDrawableIds, int[] secondarySectionsDrawableIds,
			int mainBackResId, int secondaryBackResId) {
		this(context, mainSections, secondarySections, mainSectionsDrawableIds,
				secondarySectionsDrawableIds);
	...
	public View getView(int i, View view, ViewGroup viewGroup) {
		...
		case TYPE_MAIN:
			...
			if (view == null) {
				view = View.inflate(mContext, R.layout.main_navigation_item,
						null);
				view.setBackgroundResource(mMainBackResId);
				primaryHolder = new PrimaryHolder();
				primaryHolder.primaryTextView = (CheckedTextView) view
						.findViewById(android.R.id.text1);
				primaryHolder.primaryImageView = (ImageView) view
		...
		case TYPE_SECONDARY:
			...
			if (view == null) {
				view = View.inflate(mContext,
						R.layout.secondary_navigation_item, null);
				holder = new SecondaryHolder();
				view.setBackgroundResource(mSecondaryBackResId);
				holder.secondaryTextView = (CheckedTextView) view
						.findViewById(android.R.id.text1);
				holder.secondaryImageView = (ImageView) view
						.findViewById(android.R.id.icon);
				holder.topDivider = view
						.findViewById(R.id.google_nav_drawer_divider_top);
				view.setTag(holder);
			} else {
	...
	public void setSecondarySectionsDrawableIds(
			int[] mSecondarySectionsDrawableIds) {
